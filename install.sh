...
Usage: $this [-b bindir] [-d] [-q] [-n] [-D] [tag]
  ...
  -D installs the DuckDB build when available for the current platform
   [tag] is a tag from
...
Environment variables:
  ...
  RENART_INSTALL_DUCKDB=1    Install DuckDB build when available
...
parse_args() {
  ...
  DRY_RUN=0
  RENART_INSTALL_DUCKDB="${RENART_INSTALL_DUCKDB:-0}"
...
execute() {
  ...
  if [ "${RENART_INSTALL_DUCKDB}" = "1" ] && ! asset_is_available "${ASSET_URL}"; then
    log_crit "DuckDB build is not available for ${OS}/${ARCH} on ${TAG}. Re-run without -D or set RENART_INSTALL_DUCKDB=0."
    return 1
  fi
