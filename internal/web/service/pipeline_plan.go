	...
	if s.deps.ActiveRunID != nil {
		...
		if activeErr != nil {
			base.Readiness.Warnings = append(base.Readiness.Warnings, PipelinePlanIssue{
				...
				Severity: "warning",
				Message:  "Renart could not determine whether this pipeline already has an active run",
			})
		} else if strings.TrimSpace(activeRunID) != "" {
			base.Readiness.ActiveRun = activeRunID
			base.Readiness.Blockers = append(base.Readiness.Blockers, PipelinePlanIssue{
				Code:     "pipeline_already_running",
				Severity: "error",
				Message:  "this pipeline already has a queued or running execution",
			})
		}
	}
...
func normalizePipelinePlanSelection(req PipelinePlanSelectionRequest) (PipelinePlanSelectionRequest, error) {
	...
	case PipelinePlanSelectionNeeded, PipelinePlanSelectionAll:
		if req.AssetName != "" || req.Scope != "" {
			return PipelinePlanSelectionRequest{}, fmt.Errorf("asset_name and scope are only valid for asset selection")
		}
	...
	default:
		return PipelinePlanSelectionRequest{}, fmt.Errorf("selection mode must be needed, all, or asset")
	}
	return req, nil
}
...
func PipelinePlanReviewedIdentityFromPlan(plan PipelinePlan) PipelinePlanReviewedIdentity {
	return PipelinePlanReviewedIdentity{
		...
		Selection: plan.Selection, ExecutionUnits: append([]PipelinePlanExecutionUnit(nil), plan.ExecutionUnits...),
	}
}
