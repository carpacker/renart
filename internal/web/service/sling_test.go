...
func TestSlingRunEnvIncludesResolvedIntervalDates(t *testing.T) {
	t.Parallel()
...
func TestAssetServiceCreateSlingAssetWritesFlatParamDefinition(t *testing.T) {
	...
	// A Sling asset is now a single flat-parameter .asset.yml — no .sling.yml sidecar.
	...
	assert.Contains(t, string(definition), "type: sling")
...
func TestWorkspaceServiceLoadsFlatParamSlingAsset(t *testing.T) {
	workspaceRoot := t.TempDir()
	pipelineRoot := filepath.Join(workspaceRoot, "analytics")
	...
	assert.Equal(t, "sling", asset.Type)
...
func TestWorkspaceServiceSummarizesSlingReplication(t *testing.T) {
	workspaceRoot := t.TempDir()
	pipelineRoot := filepath.Join(workspaceRoot, "analytics")
	require.NoError(t, os.MkdirAll(filepath.Join(workspaceRoot, ".git"), 0o755))
	require.NoError(t, os.MkdirAll(filepath.Join(pipelineRoot, "assets/analytics"), 0o755))
...
func TestAssetServiceDeleteSlingAssetRemovesDefinitionAndReplication(t *testing.T) {
	workspaceRoot := t.TempDir()
	pipelineRoot := filepath.Join(workspaceRoot, "analytics")
	require.NoError(t, os.MkdirAll(filepath.Join(pipelineRoot, "assets/analytics"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(pipelineRoot, "pipeline.yml"), []byte("name: analytics\n"), 0o644))
...
func TestHybridBruinExecutorRunsSlingAssetWithCLI(t *testing.T) {
	workspaceRoot := t.TempDir()
	...
	output, err := executor.runSlingAsset(context.Background(), &pipeline.Asset{
		Name:           "analytics.orders",
		Type:           pipeline.AssetType("sling"),
...
func TestHybridBruinExecutorRunsPipelineSlingAssetFromPipelineRoot(t *testing.T) {
	workspaceRoot := t.TempDir()
	pipelineRoot := filepath.Join(workspaceRoot, "analytics")
	fakeSling := filepath.Join(workspaceRoot, "fake-sling")
...
func TestHybridBruinExecutorRunsSlingAssetThroughUvWhenNoBinaryOverrideExists(t *testing.T) {
	workspaceRoot := t.TempDir()
	pipelineRoot := filepath.Join(workspaceRoot, "analytics")
