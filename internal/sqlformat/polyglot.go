...
// The embedded WASM is dist/polyglot_sql.wasm from @polyglot-sql/sdk@0.6.2.
//
...
type polyglotEngine struct {
	...
	// (it copies the module's data into fresh linear memory), so reusing instances
	// keeps SQL intellisense responsive. slots puts a hard ceiling on concurrent
	// instances, while release discards an instance only after an unusually large
	// call grows its linear memory beyond the retained-memory allowance.
...
// WebAssembly linear memory cannot shrink. Bound both the number and maximum
// size of instances, and discard an instance after a request grows it well
// beyond its normal steady-state size. This avoids periodic re-instantiation
// while ensuring one pathological query cannot permanently inflate the pool.
const (
	polyglotPoolSize                   = 4
	polyglotMemoryPageBytes     uint32 = 64 * 1024
	polyglotMaxMemoryPages             = (256 * 1024 * 1024) / polyglotMemoryPageBytes
	polyglotRetainedGrowthPages        = (16 * 1024 * 1024) / polyglotMemoryPageBytes
)
...
func callPolyglotEngine(ctx context.Context, engine *polyglotEngine, functionName string, args ...string) (resultString string, resultErr error) {
	...
	defer func() {
		if resultErr != nil {
			engine.discard(ctx, pm)
			return
		}
		engine.release(ctx, pm)
	}()
	return callPolyglotModule(ctx, pm.module, functionName, args...)
}

func callPolyglotModule(ctx context.Context, module api.Module, functionName string, args ...string) (string, error) {
...
func (engine *polyglotEngine) acquire(ctx context.Context) (*pooledModule, error) {
	...
	initialMemoryPages, _ := module.Memory().Grow(0)
	return &pooledModule{module: module, initialMemoryPages: initialMemoryPages}, nil
}

func (engine *polyglotEngine) release(ctx context.Context, pm *pooledModule) {
	defer func() { <-engine.slots }()
	if pm.module.IsClosed() {
		// WithCloseOnContextDone closes an instance when a request is canceled.
		// Never put that instance back in the idle pool even if cancellation
		// raced with an otherwise-successful guest return.
		return
	}
	currentMemoryPages, _ := pm.module.Memory().Grow(0)
	if currentMemoryPages > pm.initialMemoryPages+polyglotRetainedGrowthPages {
...
func (engine *polyglotEngine) discard(ctx context.Context, pm *pooledModule) {
	defer func() { <-engine.slots }()
	_ = pm.module.Close(ctx)
}
...
func initPolyglotInterpreter(ctx context.Context) (*polyglotEngine, error) {
	polyglotInterpreterOnce.Do(func() {
		...
		// Runtime construction initializes process-wide shared state. Do not let
		// the first abandoned editor request permanently poison sync.Once with a
		// canceled-context error; individual module calls still use and honor the
		// request context.
		buildCtx := context.WithoutCancel(ctx)
		engine, err := buildPolyglotEngine(buildCtx, wazero.NewRuntimeConfigInterpreter(), "interpreted")
		...
		if polyglotInterpreterRetired {
			...
			if engine != nil {
				_ = engine.runtime.Close(buildCtx)
			}
			return
		}
...
func buildPolyglotEngine(ctx context.Context, config wazero.RuntimeConfig, name string) (*polyglotEngine, error) {
	...
	return buildPolyglotEngineWithWASM(ctx, config, name, wasmBytes)
}

func buildPolyglotEngineWithWASM(ctx context.Context, config wazero.RuntimeConfig, name string, wasmBytes []byte) (*polyglotEngine, error) {
	config = config.
		WithMemoryLimitPages(polyglotMaxMemoryPages).
	...
	return &polyglotEngine{
		...
		slots:    make(chan struct{}, polyglotPoolSize),
	}, nil
}
