  ...
  return (
    ...
    <div className="flex h-dvh min-h-0 flex-col bg-muted/40 text-foreground">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-60">
              ...
              <DropdownMenuItem asChild>
                <Link to="/account/profile"><User className="size-4" />Account settings</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link to="/account/members"><Users className="size-4" />Members & permissions</Link>
              </DropdownMenuItem>
              <DropdownMenuItem asChild>
                <Link to="/account/billing"><CreditCard className="size-4" />Billing</Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem>
      ...
      <nav className="grid h-14 shrink-0 grid-cols-5 border-t bg-background pb-[env(safe-area-inset-bottom)] md:hidden">
        {navItems.map((item) => (
          <Link key={item.to} to={item.to} activeOptions={{ exact: item.to === "/" }} activeProps={{ className: "text-primary" }} className="flex flex-col items-center justify-center gap-0.5 text-[10px] text-muted-foreground">
...
function DiffViewer({ diff, loading, className }: { diff: { path: string; staged: boolean; patch: string } | null; loading: boolean; className?: string }) {
  if (loading) {
...
function diffLineClassName(line: string, hasPatch: boolean) {
  ...
  if (line.startsWith("diff --git") || line.startsWith("index ")) return "bg-sky-500/10 text-sky-200";
