...
export function normalizeAppRunsSearch(search: Record<string, unknown>): AppRunsSearch {
  ...
  return {
    ...
    status: runStatuses.includes(search.status as never) ? (search.status as AppRunsSearch["status"]) : undefined,
    page,
  };
}

export function AppRunsPage({ search = {}, onSearchChange }: { search?: AppRunsSearch; onSearchChange?: (search: AppRunsSearch) => void }) {
  const q = search.q ?? "";
        ...
        <AppPanel className="flex h-full min-h-0 flex-col overflow-hidden">
          <SimpleTable
            ...
            rows={visibleRuns.map((run) => [
              ...
              <Link key="id" to="/runs/$runId" params={{ runId: run.id }} search={search} className="font-mono text-primary hover:underline">{run.id}</Link>,
              ...
              <Button key="open" asChild variant="ghost" size="icon-sm"><Link to="/runs/$runId" params={{ runId: run.id }} search={search}><ChevronRight className="size-4" /></Link></Button>,
            ])}
          />
...
export function AppRunDetailPage({ runId, search = {} }: { runId: string; search?: AppRunsSearch }) {
      ...
      <PageHeader
        ...
        actions={(
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="icon-sm"><Link to="/runs" search={search}><ArrowLeft className="size-4" /></Link></Button>
