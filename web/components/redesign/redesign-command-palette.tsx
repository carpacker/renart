...
export function RedesignCommandPalette() {
  ...
  const pageItems = useMemo<PaletteItem[]>(() => {
    ...
    return [
      { id: "page:build", title: "Build", subtitle: "Pipeline canvas & editor", icon: Hammer, to: () => navigate({ to: "/redesign" }) },
      { id: "page:catalog", title: "Catalog", subtitle: "Asset lineage across the project", icon: Network, to: () => navigate({ to: "/redesign/catalog" }) },
      { id: "page:notebooks", title: "Notebooks", subtitle: "Exploratory DuckDB notebooks", icon: BookOpen, to: () => navigate({ to: "/redesign/notebooks" }) },
      { id: "page:runs", title: "Runs", subtitle: "Pipeline run history", icon: Play, to: () => navigate({ to: "/redesign/runs" }) },
      { id: "page:schedules", title: "Schedules", subtitle: "Scheduled pipeline runs", icon: Calendar, to: () => navigate({ to: "/redesign/schedules" }) },
      { id: "page:environments", title: "Environments", subtitle: "Project settings", icon: Settings2, to: () => navigate({ to: "/redesign/project/environments" }) },
      { id: "page:connections", title: "Connections", subtitle: "Project settings", icon: Settings2, to: () => navigate({ to: "/redesign/project/connections" }) },
  ...
  const sections = useMemo<PaletteSection[]>(() => {
    ...
    for (const pipeline of workspace?.pipelines ?? []) {
      pipelineItems.push({
        ...
        perform: () => {
          void navigate({ to: "/redesign/pipelines/$pipelineId/canvas", params: { pipelineId: pipeline.id } });
          close();
        },
      });
    ...
    for (const notebook of workspace?.notebooks ?? []) {
      notebookItems.push({
        ...
        perform: () => {
          void navigate({ to: "/redesign/notebooks/$notebookId", params: { notebookId: notebook.id } });
          close();
        },
      });
    }
