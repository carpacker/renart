...
import { discoverSlingStreams, SlingDiscoveryStream } from "@/lib/api-sling";
...
// A Sling source/target can be a local file rather than a bruin connection. It is
...
/**
 * The main-pane editor for a Sling asset. Sling assets carry their whole
 ...
 */
export function SlingParametersEditor({ asset, pipelineId }: { asset: WebAsset; pipelineId: string }) {
  ...
  const connections = useMemo(() => {
    ...
    const active =
      ...
      environments[0];
    const slingConnections = (active?.connections ?? []).filter((connection) =>
      Boolean(connection.sling_category)
    );
    // The local-file pseudo-connection is always available.
    return [LOCAL_CONNECTION_OPTION, ...slingConnections];
  ...
  return (
    <div className="font-monaco min-h-0 flex-1 overflow-y-auto bg-background p-3 text-[13px] leading-6">
      ...
      <Line>
        <Key>type</Key>
        <span className="text-foreground">sling</span>
      </Line>
      <Line>
        <Key>parameters</Key>
      </Line>
...
// is grouped by Sling category (database / storage / file).
function ConnectionValue({
  ...
  const grouped = useMemo(() => {
    ...
    for (const connection of connections) {
      const category = connection.sling_category ?? "database";
            ...
            {grouped.map((group) => (
              <CommandGroup key={group.category} heading={CATEGORY_LABELS[group.category] ?? group.category}>
                {group.items.map((connection) => {
                  const Icon = categoryIcon(connection.sling_category);
...
function StreamValue({
  ...
  const [streams, setStreams] = useState<SlingDiscoveryStream[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    discoverSlingStreams({ connection, environment, signal: controller.signal })
      .then((result) => {
