...
function statusForStep(status: PipelineRunStep["status"]): Exclude<AppAssetMaterializationStatus, "unknown"> {
...
function keysForStepAsset(stepAsset: string, assets: AppMaterializationAsset[], pipelineId?: string) {
...
export function labelForAppMaterializationState(state?: AppAssetMaterializationDisplayState) {
  ...
  if (state.status === "failed") return state.materializedAt ? formatAppMaterializedAt(state.materializedAt) : "failed";
  return "no run yet";
}
