...
test.describe("notebook python logs", () => {
  ...
  test("python stdout is captured and shown as collapsible output", async ({ liveApp, page }) => {
    ...
    await page.goto(`${liveApp.baseURL}/redesign/notebooks/${notebook.id}`);
    ...
    await page.screenshot({ path: "/home/lukas/.claude/jobs/a2129ded/tmp/notebook-pylogs.png", fullPage: true });
  });
});
