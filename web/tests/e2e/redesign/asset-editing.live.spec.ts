...
test.describe("redesign asset editing workbench live", () => {
  test.use({ fixtureName: "configured-workspace" });

  test("guided cards render and adding a manual dependency persists provenance", async ({ liveApp, page }) => {
    await page.goto(`${liveApp.baseURL}/redesign/pipelines/${pipelineId}/assets/${customersAssetId}/code`);
  ...
  test("materializing an asset on a stale upstream warns before building", async ({ liveApp, page }) => {
    ...
    await page.goto(`${liveApp.baseURL}/redesign/pipelines/${pipelineId}/assets/${warnDownId}/code`);
    await expect(page.locator(".monaco-editor").first()).toBeVisible({ timeout: 15000 });
    await staleness;
