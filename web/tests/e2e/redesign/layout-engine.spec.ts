...
import { computeRedesignLineageLayout, type RedesignLineageLayoutEdge, type RedesignLineageLayoutNode } from "../../../lib/redesign-lineage-layout";

function node(id: string): RedesignLineageLayoutNode {
  const [layer, ...nameParts] = id.split(".");
  return { id, layer, name: nameParts.join(".") || id };
}

function edge(source: string, target: string): RedesignLineageLayoutEdge {
  return { source, target };
}

test.describe("redesign lineage layout engine", () => {
