...
test.describe("redesign build editor live", () => {
  ...
  test("edits an asset in Monaco and persists the change", async ({
    ...
    expect(customers?.content).toContain("-- redesign editor smoke");
  });
...
`,
    ...
    await page.goto(
      `${liveApp.baseURL}/redesign/pipelines/${pipelineId}/assets/${customerStatsAssetId}/code`
    );
